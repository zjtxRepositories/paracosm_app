  ...
  Widget _buildHeader(BuildContext context, AppLocalizations l10n) {
    return Padding(
      ...
      child: Row(
        ...
        children: [
          Text(
            l10n.translate('moments_moment_title'),
  ...
  Widget _buildStories(BuildContext context) {
    final stories = <_StoryData>[
      ...
      _StoryData(
        ...
        showAddBadge: true,
        onTap: () => context.push('/moment-user-profile?mode=self'),
      ),
      ...
      _StoryData(
        ...
        hasUpdate: true,
        onTap: () => context.push('/moment-user-profile?mode=friend'),
      ),
      const _StoryData(
        label: 'Kristen',
        avatar: 'assets/images/chat/avatar.png',
...
class _StoryAvatarState extends State<_StoryAvatar>
  ...
  void initState() {
    super.initState();
    if (widget.story.hasUpdate) {
      // 仅为有新动态的头像添加轻量脉冲动画，避免影响页面其它区域。
  ...
  Widget _buildAvatar(BuildContext context, double pulse) {
    final outerColor = widget.story.outsideColor;
    final innerColor = Color.lerp(
      widget.story.insideColor,
      const Color(0xFFDFFF70),
      pulse * 0.45,
    )!;
          ...
          child: Padding(
            ...
            child: Stack(
              ...
              children: [
                ...
                if (widget.story.showPlusIcon)
                  const Center(
                    child: Icon(Icons.add, size: 18, color: AppColors.grey600),
                  ),
                if (widget.story.showAddBadge)
                  Positioned(
                    right: -1,
                    bottom: -5,
                    child: GestureDetector(
  ...
  @override
  Widget build(BuildContext context) {
    final avatar = widget.story.hasUpdate && _pulse != null
        ? AnimatedBuilder(
            animation: _pulse!,
            builder: (context, child) {
              return _buildAvatar(context, _pulse!.value);
            },
          )
...
class _MomentPostCard extends StatelessWidget {
  ...
  void _showShareModal(BuildContext context) {
    ...
    AppModal.show(
      context,
      title: l10n.translate('moments_share'),
      confirmText: null,
      onConfirm: () {},
